...
### Требования
- Vector + ElasticSearch + Kibana для сбора логов всех сервисов API Gateway
- Docker Compose файл
- Kibana доступна на http://localhost:8081
- Учетные данные: admin/qwerty123456

### Реализация

Развернут стек логирования на GCP: http://34.31.7.74:8081

**Компоненты:**
- **Vector 0.35.0** - сбор логов из Docker контейнеров
- **ElasticSearch 8.11.3** - хранение и индексирование логов
- **Kibana 8.11.3** - веб-интерфейс для поиска и анализа

**Конфигурация:** [task-04-logging/](task-04-logging/)

**Возможности:**
- ✅ Автоматический сбор логов из stdout всех контейнеров (api-gateway, security-service, uploader-service, minio)
- ✅ Обогащение логов метаданными (service, service_type, log_level, environment)
- ✅ Автоматическая категоризация по уровням (error, warn, info, debug)
- ✅ Индексация в ElasticSearch: `microservices-logs-YYYY.MM.DD`
- ✅ Поиск и фильтрация в Kibana
- ✅ Гарантированная доставка через buffering
...
- Метрики с сервисов: security, uploader, minio
- Dashboard с распределением запросов по сервисам
- Docker Compose файл
- Grafana на http://localhost:8082
- Учетные данные: admin/qwerty123456

### Реализация

Развернут стек мониторинга на GCP:
...
- **Node Exporter 1.7.0** - системные метрики хоста (CPU, RAM, HDD, Network)
- **cAdvisor 0.47.2** - метрики Docker контейнеров

**Конфигурация:** [task-05-monitoring/](task-05-monitoring/)
...
2. **Метрики контейнеров** (cAdvisor):
   - CPU usage по каждому контейнеру
   - Memory usage по контейнеру
...
Автоматически созданный dashboard включает:

1. **Requests Rate by Service** - скорость запросов (bytes/sec) по каждому сервису
2. **Container CPU Usage** - использование CPU в процентах
3. **Container Memory Usage** - использование памяти в байтах
4. **Network I/O by Service** - входящий и исходящий трафик
5. **Request Distribution (Pie Chart)** - процентное распределение запросов между сервисами
6. **System Load Average** - загрузка системы (1m, 5m, 15m)
...
# Memory usage
container_memory_usage_bytes{name=~"security-service|uploader-service|api-gateway|minio"}
...
```bash
# Запуск стека
cd task-05-monitoring
docker compose up -d

# Grafana
# URL: http://localhost:8082
# Login: admin / qwerty123456
# Dashboard: Microservices Request Distribution
...
### Логирование: Vector + ElasticSearch + Kibana (реализовано)
- ✅ Развернуто на GCP: http://34.31.7.74:8081
- ✅ Автоматический сбор логов из всех сервисов
- ✅ Индексация и полнотекстовый поиск
- ✅ Обогащение метаданными и категоризация

### Мониторинг: Prometheus + Grafana (реализовано)
...
Все три компонента интегрируются в единую observability платформу, что обеспечивает:
...
Полная инфраструктура развернута с помощью Terraform и Docker Compose:

```bash
# 1. Создание VM на GCP
cd terraform
terraform init
terraform apply

# 2. Деплой всех стеков (основные сервисы + логи + мониторинг)
./deploy-to-gcp.sh <VM_IP>
```

**Результат:** Работающая система на http://34.31.7.74 со всеми компонентами:
- API Gateway и микросервисы
- Система логирования (Kibana на порту 8081)
- Система мониторинга (Grafana на порту 8082)
